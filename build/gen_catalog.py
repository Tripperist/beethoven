...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .site-nav {{ display: flex; justify-content: center; flex-wrap: wrap; gap: 1.75rem; margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1); }}
  .site-nav-item {{ display: flex; flex-direction: column; align-items: center; gap: 0.25rem; text-decoration: none; max-width: 220px; }}
  .site-nav-label {{ font-size: 0.82rem; font-weight: 600; letter-spacing: 0.02em; color: var(--gold-light); }}
  a.site-nav-item:hover .site-nav-label {{ color: var(--paper); }}
  .site-nav-item.current .site-nav-label {{ color: var(--paper); }}
  .site-nav-desc {{ font-size: 0.72rem; color: var(--ink-faint); line-height: 1.45; }}
  ...
  .toc-band {{ background: var(--paper); border-bottom: 1px solid var(--rule-light); padding: 0.9rem 2rem; position: sticky; top: 0; z-index: 10; backdrop-filter: blur(6px); background: rgba(247,243,237,0.94); }}
  .toc-inner {{ max-width: 900px; margin: 0 auto; display: flex; flex-wrap: nowrap; gap: 1.1rem; overflow-x: auto; scrollbar-width: none; }}
  .toc-inner::-webkit-scrollbar {{ display: none; }}
  .toc-inner a {{ font-size: 0.78rem; letter-spacing: 0.05em; text-transform: uppercase; color: var(--ink-muted); text-decoration: none; border-bottom: 1px solid transparent; padding-bottom: 2px; white-space: nowrap; flex-shrink: 0; }}
  ...
  /* Kept in sync with .toc-band's fixed single-row height (padding + one text line) */
  .city-section {{ margin-bottom: 3.5rem; scroll-margin-top: 4.25rem; }}
  ...
  .entry {{ background: white; border: 1px solid var(--rule-light); border-radius: 2px; padding: 1.4rem 1.6rem; margin-bottom: 1rem; scroll-margin-top: 4.25rem; }}
...
<header class="header">
  ...
  <nav class="site-nav">
    ...
    <a class="site-nav-item" href="index.html">
